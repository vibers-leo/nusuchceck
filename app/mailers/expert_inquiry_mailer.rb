class ExpertInquiryMailer < ApplicationMailer
  ADMIN_EMAIL = ENV.fetch("SMTP_USERNAME", "juuuno1116@gmail.com")

  # 문의 접수 시 관리자에게
  def inquiry_received(inquiry)
    @inquiry = inquiry
    mail(to: ADMIN_EMAIL, subject: "[누수체크] 전문가 등록 문의 - #{inquiry.name}")
  end
  ...
  def inquiry_received_raw(name:, phone:, email:, message:)
    ...
    mail(to: ADMIN_EMAIL, subject: "[누수체크] 전문가 등록 문의 - #{name}")
  end
